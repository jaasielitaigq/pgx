...
//
// Acquire guarantees that even if ctx is canceled concurrently with a successful resource creation, the resource will
// be properly returned to the idle pool (or destroyed) and the semaphore count will remain consistent.
...
func (p *Pool[T]) acquire(ctx context.Context) (*Resource[T], error) {
	...
	// If the context has been cancelled between acquiring the semaphore and
	// attempting to use it, release the semaphore and return immediately.
	// This prevents goroutine leaks when the caller's context is cancelled
	// concurrently with a TryAcquire success.
	select {
	case <-ctx.Done():
