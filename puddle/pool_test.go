...
func TestPoolAcquireStressCancelContention(t *testing.T) {
	t.Parallel()

	constructor, _ := createConstructor()
	pool, err := puddle.NewPool(&puddle.Config[int]{Constructor: constructor, Destructor: stubDestructor, MaxSize: 3})
	require.NoError(t, err)
	defer pool.Close()
	...
	require.NoError(t, err)
	res3, err := pool.Acquire(context.Background())
	require.NoError(t, err)
	...
	for i := 0; i < 30; i++ {
		...
		go func() {
			defer wg.Done()
			for j := 0; j < 10; j++ {
				ms := time.Duration(rand.Intn(5)+1) * time.Millisecond
	...
	defer cleanCancel()
	res, err := pool.Acquire(cleanCtx)
	require.NoError(t, err, "pool should be functional after cancellation chaos")
	res.Release()

	stat := pool.Stat()
	t.Logf("Total: %d, Acquire: %d, Canceled: %d, Idle: %d",
		stat.TotalResources(), stat.AcquireCount(), stat.CanceledAcquireCount(), stat.IdleResources())
	require.Equal(t, cancelCount.Load(), stat.CanceledAcquireCount(),
		"canceled acquire count mismatch")
	require.GreaterOrEqual(t, stat.IdleResources(), int32(0),
		"idle resources should never be negative")
	require.GreaterOrEqual(t, stat.TotalResources(), int32(1),
		"pool should have resources available")
	require.LessOrEqual(t, stat.TotalResources(), int32(3),
